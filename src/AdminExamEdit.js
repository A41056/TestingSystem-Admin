                ...
                <div className="card mb-3" key={questionIndex}>
                    <div className="card-body">
                        ...
                        <select className="form-select mb-2" value={question.isSingleChoice.toString()} onChange={(e) => handleQuestionTypeChange(e, questionIndex)}>
                            ...
                            <option value="false">Multiple Choices</option>
                        </select>
                    <button className="btn btn-sm btn-outline-primary mb-2 me-2" onClick={() => handleAddAnswer(questionIndex)}>Add Answer</button>
                    {question.answers.map((answer, answerIndex) => (
                        <div className="input-group mb-2" key={answerIndex}>
                            ...
                            <button className="btn btn-sm btn-outline-danger" onClick={() => handleRemoveAnswer(questionIndex, answerIndex)}>Remove Answer</button>
                        </div>
                    ))}
                    <button className="btn btn-sm btn-outline-danger me-2" onClick={() => handleRemoveQuestion(questionIndex)}>Remove Question</button>
                </div>
            </div>
        ))}
        <button className="btn btn-sm btn-outline-success mb-3 me-2" onClick={handleAddQuestion}>Add Question</button>
        <button className="btn btn-sm btn-primary mb-3 me-2" onClick={handleSaveExam}>Save Exam</button>
        <Link className="btn btn-sm btn-outline-secondary mb-3 me-2" to="/admin-exams" style={{ verticalAlign: 'middle' }}>Back</Link>
